...
Blog.Content.renderRecentPosts()};Blog.Data={};Blog.Data.getIndexDataUrl=function(){return Blog.Settings.DataRoot+"index.json"};Blog.Data.getPostDataUrl=function(a){var b=Blog.Navigation.getPostIndex(a),c=Blog.Data.parseIsoDate(Blog.PostData.Index[b].date);b=c.getMonth()+1;c=c.getFullYear();return Blog.Settings.DataRoot+c+"/"+(10>b?"0"+b:b)+"/"+a+".json"};Blog.Data.load=function(a){Blog.PostData.Index=a.posts;Blog.PostData.TagCloud=a.tags};
...
Blog.Content.renderRandom=function(){var a=Blog.Navigation.buildSinglePostLink(Blog.PostData.Index[Math.floor(Math.random()*Blog.PostData.Index.length)].id);location.hash=a};Blog.PostData={};